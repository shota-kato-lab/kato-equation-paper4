% ============================================================
%
% V205 (2026-05-15, paper4_polish-ya_5 wave 7 = Paper 4 fate decided + new title per author 5/15 chat + xlsx canonical):
%   * Author directive verbatim: "paper4 処遇がやっと決まった！ タイトル変更！"
%   * xlsx canonical: H_era/paper_titles/Paper_Titles_and_Submission_Strategies_V1.xlsx Sheet1 row 4 (Paper 4 ✅)
%   * Old title (V200-V204): "Paper 4: The Kato Relay-Depth Ladder \\ Closing the Bettencourt 22-Indicator Panel \\[1pt] {\Large\itshape A zero-parameter exponent rule for urban scaling}"
%   * New title (V205+): "A Relay-Depth Exponent Ladder for the Bettencourt 22-Indicator Urban-Scaling Panel \\[1pt] {\Large\itshape A Zero-Fitted-Exponent Complementary Closure with Integer, Continuous, and Hybrid Assignments}"
%   * Length: Main 82 chars, Subtitle 93 chars (xlsx audit)
%   * Submission strategy: Nature Cities first / SA alt -> npj Complexity -> EPJ-B fallback (PNAS Nexus drop)
%   * Key changes:
%     - Drop "Paper 4: The Kato" prefix (= author-neutral framing for journal submission, drop series identity)
%     - Drop "Closing" -> "for" (= less aggressive framing, "for" emphasizes service)
%     - Drop "Panel" -> "Urban-Scaling Panel" (= clarify scope explicit)
%     - Subtitle restructure: "A zero-parameter exponent rule for urban scaling" -> "A Zero-Fitted-Exponent Complementary Closure with Integer, Continuous, and Hybrid Assignments" (= explicit 3-tier framework reference)
%   * V204 cumulative fixes ALL maintained: 197d/198/199/200b/204/205/210/211/213/214/215/217/218/219/220a/220b
%   * FIX 221 NEW: title change per author + xlsx canonical
%   * Page 1 banner stamp NOT touched (V200 cover keep maintained)
%   * Central claim 22/22 unchanged, Kato ladder integers unchanged
%   * V201 untouched (absolute prohibition maintained)
% ============================================================
% ============================================================
%
...
%   FIX 175 *** Banner V194 -> V195 + page 1 banner -> "Preprint version 1.0 (2026-05-15)" (P4polish_014 §2(a)#2)
...
%     - 既存 page 1 bottom-right rendering: "Shota Kato ・ AI and Future Co., Ltd., Tokyo, Japan ・ Independent research ・ shota.kato@aiandfuture.co.jp"
%     - 新: メルアド掲載は corresponding author footnote のみ retain、 page 1 bottom-right は AI and Future Co., Ltd. 部分に URL link inject (https://aiandfuture.co.jp)
...
\title{A Relay-Depth Exponent Ladder for the Bettencourt 22-Indicator Urban-Scaling Panel \\[1pt] {\Large\itshape A Zero-Fitted-Exponent Complementary Closure with Integer, Continuous, and Hybrid Assignments}}
...
\vspace{12pt}
{\centering\small\itshape Kato Equation Working Paper Series \quad|\quad Preprint version 1.0 (2026-05-15)\par}
\vspace{0.4em}
...
\vspace{1.5em}
{\centering\scriptsize\itshape Shota Kato \quad\textbullet\quad \href{https://aiandfuture.co.jp}{AI and Future Co., Ltd., Tokyo, Japan} \quad\textbullet\quad Independent research\par}
\vspace{0.4em}
...
\clearpage
\newgeometry{top=0.65in, bottom=1.4cm, left=1in, right=1in, footskip=0.8cm}  % V205 FIX 200b: page 2+ top 16.5mm
\doublespacing
...
This is a single-author manuscript. S.K.\ is responsible for all stages of the work and for the manuscript text.
...
% V205 FIX 197d+218: tabcolsep 1pt + arraystretch 1.05 -> 0.95
...
\multirow{17}{*}{\rotatebox[origin=c]{90}{\textbf{Integer-rung tier (17/22)}}}
 ...
 & 12 & GDP (US, OECD)$^{\dagger\dagger}$ & reference & ($+$) $\{3,4,5\}$ mix, $\Heff{\approx}4.42$ & \shortstack{$1.099\sim$\\$1.114$} & 1.13$\sim$1.26 & 1920 & \mbox{1751$\sim$1971} \\
 & 13 & GDP (US 1969--2003) & strict & ($+$) $\{3,4,5\}$ mix, $\Heff\approx4.42$ & 1.099 & 1.099 (MSA) & \shortstack[l]{1920 carrier\\1986 obs.} & \mbox{1751$\sim$1971} \\
...
Paper~4 measures the Kato relay-depth \(H\) within the Paper~3 \(\Omega\)-domain, hence its scope is bounded by the natural span of \(\Omega\equiv\int N^{2}\,dt\) before the post-1971 substrate handoff (see the Scope-boundary paragraph in the Discussion above). This appendix retains the pre-1971 portion of the \(H\)-era ladder as the temperature-measurement reference table for the Paper~4 main body. The retained rungs run from the hunter--gatherer \(H{=}1\) era through the H${=}$4$\to$5 silent-Kramers corridor that ends at the 1971 TFR-peak lock. The post-1971 substrate-relay era (Paper~5 territory: H${=}$5$\to$6 around 2010, H${=}$6 around 2017--2024 hyperscaler DC AI, H${=}$6$\to$7 AGI emergence, and beyond) is removed from this appendix; readers requiring the post-1971 cascade evidence should consult Paper~5 Supplementary Information.
...
% V205 FIX 210+217: H_era REVERSE Substrate p{7.5cm} + Predicted beta+ p{2.2cm} + arraystretch 1.1
{\scriptsize
\setlength{\tabcolsep}{1pt}
...
Throughout Paper~4 the 1971 transition is labelled consistently as \(H=4\to5\) under the cluster-mixture central-\(H\) framework, in agreement with the Paper~3 alternative wholesale-adoption interpretation of 1751--1971 as a silent \(H=4\to5\) Kramers corridor culminating at the 1971 TFR-peak lock. The 1820 boundary is labelled as the industrial-onset corridor entry within the same \(H=4\to5\) silent-Kramers reading. The AD~151 boundary is labelled at the Roman-peak end of the antiquity (\(H=2\)) phase. No stray \(H=3\to4\) labelling is used for any post-1971 phenomenon.

\paragraph{Withdrawal of the previously proposed numerical \(\beta\)-bridge.}
The previously proposed numerical bridge from \(H\)-era rungs to a single-population \(\beta\) trajectory is withdrawn. The USPTO inventor-fractional audit does not support that bridge and therefore cannot be used as a Paper~4 validation. The audit concerns a Bettencourt-style log--log slope for a specified output, geography, and population denominator; it is not a direct measurement of the latent cluster-mixture \(H\) state. Paper~4 retains only the AD~151 / 1820 / 1971 cluster-mixture boundary spine and the pre-1971 H${=}$1$\to$4$\to$5 rung table above. The bridge is not replaced by a post-hoc inversion, and no revised single-population \(\beta\) ladder is released in Paper~4; any such ladder, if it is ever developed, is the responsibility of Paper~5 SI and Paper~3 SI.

\paragraph{The $\Omega$-era catalogue and its cross-paper layering.}
The ten-phase $\Omega$-era catalogue used in this paper---$\Omega_{\rm competitive\_coop}$, $\Omega_{\rm territory\_tool}$, $\Omega_{\rm food\_pre\_irrig}$, $\Omega_{\rm food\_post\_irrig}$, $\Omega_{\rm dark\_age}$,\linebreak $\Omega_{\rm empire\_writing}$, $\Omega_{\rm black\_death}$, $\Omega_{\rm capital\_printing}$, $\Omega_{\rm industry\_science}$, $\Omega_{\rm longlife\_computing}$---is the cluster-standard ten-phase classification, also adopted by the substrate-relay cascade discussion of Paper~5~\cite{Kato2026e} and by a cross-paper $H_{\rm era}$ reference dataset whose extended sixteen-$\Omega$ ledger (pre- and post-1971 expansion) is developed in an in-preparation companion document (forthcoming). This paper uses the ten-phase form because the 1971 boundary employed here lies precisely at the $\Omega_{\rm industry\_science}\!\to\!\Omega_{\rm longlife\_computing}$ transition.

\paragraph{Beyond 1971: digital-asset and post-1971 forward reference.}
The post-1971 digital-asset substrate (Ethereum and DeFi protocols) and the associated $H{=}6$ anchor evidence, together with the post-1971 USPTO inventor-fractional $\beta$ audit, are developed in the substrate-relay cascade theory of Paper~5~\cite{Kato2026e} and its Supplementary Information; none of that post-1971 evidence is used in the present paper to support the pre-1971 chronology, in keeping with the scope rule already stated above.
