...
{\centering\small\itshape Kato Equation Working Paper Series \quad|\quad Version V119 (2026-05-11)\par}
\vspace{0.4em}
...
\begin{abstract}
The Kato relay-depth ladder $\beta_\pm(H)=1\pm 1/[H\ln(2H+1)]$ closes Bettencourt's canonical 22-indicator urban panel at the 95\,\% CI with zero fitted exponents: 17/22 at strict integer $H$, 21/22 with continuous $\Heff$, 22/22 under the label-free K=2 lower-valley mixture $H{=}\{1,2\}$ for physical $d{=}3$ cities (MAE = 0.031, RMSE = 0.045). The framework is falsifiable: a drop to 21/22 or below disconfirms the central 22/22 closure.
\end{abstract}
...
\centering
\(\displaystyle
\beta_{\pm}(H) \;=\; 1 \;\pm\; \frac{1}{H \ln(2H+1)} \\[4pt]
\beta_{\rm obs}^{(j)} \;=\; \pi_{1}^{(j)}\,\beta_{+}(1) \;+\; \pi_{2}^{(j)}\,\beta_{+}(2) \quad \text{(lower-valley K=2 mixture)}
\)
\end{tcolorbox}
\vspace{-0.3em}
...
& {\fontsize{11pt}{13pt}\selectfont\textbf{[Academic Primer]} The formal entry point for Papers~1--10. Provides the cluster-wide notation, the $\Omega{\leftrightarrow}H$ translation table linking substrate eras to integer rungs, the relay-depth ladder derivation $\beta_\pm(H){=}1{\pm}1/[H\ln(2H{+}1)]$, three reading paths (academic, popular, reviewer), and the citation conventions used across the cluster.} \\
\specialrule{1.5pt}{0pt}{0pt}
\end{tabular}
\\[3mm]
...
\specialrule{1.5pt}{0pt}{0pt}
\textbf{Paper 1} & ``Singularity abundance'' narrative is refuted: even with full Kato extrapolation through 2066, world GDP per capita grows only $+27\%$ (about 0.5\,\%/yr) from the 2024 baseline. & Closed-form world GDP--population law over 200 years (1820--2024, $R^2{=}0.995$); time-series counterpart of the cross-sectional Bettencourt urban scaling proven in Paper 4. \\
\hline
...
which resolves to the latest manuscript snapshot;
all versioned releases (including v1.0-bettencourt and the present v1.1
manuscript) are listed on the Zenodo record page.
...
\begin{tabular}{p{3.6cm}p{2.9cm}p{2.0cm}p{3.5cm}}
\toprule
...
3 & 734--1221 & Carolingian/\allowbreak Byzantine/\allowbreak Abbasid/\allowbreak Tang--Song (Monasterium, Rashdall, Skinner, Ge) & 1.083 & N$\geq$30 polities \\
4 & 1472--1750 & Printing+commerce (USTC/ESTC, Sound Toll, Amsterdam, BoE) & 1.058 & N$\geq$30 European cities \\
4$\to$5 & 1750--1970 & Industrial/\allowbreak electrical/\allowbreak petroleum (USPTO, EIA/IEA, BP, OWID) & 1.05 (silent corridor) & N$\geq$30 states/countries \\
...
\paragraph{Substrate denominator drift caveat (V108 update, Paper 5 V44 cross-reference).}
Per-capita energy and per-capita compute proxies for $H_{\mathrm{est}}$ become inflated for $t \geq 1970$ because the dissipation-bearing substrate population shifts from $N_{\mathrm{human}}$ alone to $N_{\mathrm{human}} + w_{\mathrm{compute}} N_{\mathrm{AGI}}$ as autonomous agents and AI compute infrastructure absorb a growing share of the energy flux. Under the human-only denominator, $H_{\mathrm{est}} \to \infty$ in the limit $N_{\mathrm{human}} \to 1$, an apparent ``Singularity'' that is dissolved once the substrate-aware denominator is adopted, restoring a bounded $H_{\mathrm{est}}$ trajectory across the brain $\to$ AGI substrate handoff (Paper 5 V44 \S\,Singularity-as-perceptual-artifact). For the urban scaling cross-section in this paper, this caveat is benign: the Bettencourt 22-indicator panel's per-MSA labour and resident counts both index the same human-substrate denominator within each indicator's own normalisation, so the 22/22 closure result is unaffected. The caveat is recorded here for cross-paper consistency with the multi-substrate framework documented in Paper 5 V44 and the $H_{\mathrm{era}}$ ladder validation figure (Paper 5 main Figure 2; reproduced as Paper 4 SI inset reference).
